...
from denoisr_chess.data.stockfish_oracle import StockfishOracle
...
def _cleanup_oracle() -> None:
    ...
    try:
        _oracle.close()
    except (chess.engine.EngineError, OSError, RuntimeError):
...
@graceful_main("denoisr-chess-generate-data", logger=log)
def main() -> None:
    load_env_file()
