...
from denoisr_chess.evaluation.analyzer import AnalysisResult, run_analysis
...
from denoisr_chess.evaluation.pgn_writer import write_combined_pgn, write_pgn
from denoisr_common.interrupts import graceful_main
from denoisr_common.runtime import build_parser, configure_logging, load_env_file
...
def _default_openings_path() -> Path:
    ...
    ref = resources.files("denoisr_chess.data.openings").joinpath("default.epd")
    return Path(str(ref))
...
def _make_on_game(label: str, total_games: int) -> callable:
    def on_game(played: int, wins: int, draws: int, losses: int) -> None:
        from denoisr_chess.engine.elo import compute_elo
...
@graceful_main("denoisr-chess-benchmark", logger=logger)
def main() -> None:
    load_env_file()
