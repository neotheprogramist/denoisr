...
from denoisr_chess.training.dataset import ChessDataset
from denoisr_chess.training.ema import ModelEMA
from denoisr_chess.training.grok_tracker import GrokTracker
...
from denoisr_chess.training.resource_monitor import ResourceMonitor
from denoisr_chess.training.plateau_detector import PlateauDetector
from denoisr_chess.training.prefetch import DevicePrefetcher
...
from denoisr_chess.types.training import PolicyTarget, ValueTarget
...
@graceful_main("denoisr-chess-train-phase1", logger=log)
def main() -> None:
    load_env_file()
    ...
    add_env_argument(
        ...
        help="Checkpoint to load (create with denoisr-chess-init)",
    )
    add_env_argument(
        parser,
        "--data",
        env_var="DENOISR_PHASE1_DATA",
        help="Path to training data .pt file (create with denoisr-chess-generate-data)",
    )
    add_env_argument(
        parser,
        "--holdout-frac",
