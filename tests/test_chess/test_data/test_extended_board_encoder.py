...
class TestExtendedBoardEncoder:
    ...
    def test_num_planes_exceeds_simple(self, encoder: ExtendedBoardEncoder) -> None:
        assert encoder.num_planes > 12

    def test_starting_position_shape(self, encoder: ExtendedBoardEncoder) -> None:
    ...
    def test_castling_rights_change(self, encoder: ExtendedBoardEncoder) -> None:
    ...
    def test_en_passant_encoded(self, encoder: ExtendedBoardEncoder) -> None:
        board = chess.Board()
    ...
    def test_side_to_move_differs(self, encoder: ExtendedBoardEncoder) -> None:
    ...
    def test_deterministic(self, encoder: ExtendedBoardEncoder) -> None:
        board = chess.Board()
    ...
    def test_no_nan_or_inf(self, board: chess.Board) -> None:
    ...
    @given(board=random_boards())
    @settings(max_examples=30)
    def test_values_bounded(self, board: chess.Board) -> None:
        encoder = ExtendedBoardEncoder()
        bt = encoder.encode(board)
        assert bt.data.min() >= 0.0
        assert bt.data.max() <= 1.0

    def test_produces_122_planes(self, encoder: ExtendedBoardEncoder) -> None:
        bt = encoder.encode(chess.Board())
        assert bt.data.shape == (122, 8, 8)

    def test_attack_planes_nonzero(self, encoder: ExtendedBoardEncoder) -> None:
    ...
    def test_attack_planes_match_piecewise_union(
        self, encoder: ExtendedBoardEncoder
    ) -> None:
        board = chess.Board()
        bt = encoder.encode(board)

        for plane_idx, color in ((110, chess.WHITE), (111, chess.BLACK)):
            expected = 0
            for sq, piece in board.piece_map().items():
                if piece.color == color:
                    expected |= board.attacks_mask(sq)

            actual = 0
            for rank in range(8):
                for file in range(8):
                    if bt.data[plane_idx, rank, file] > 0:
                        actual |= chess.BB_SQUARES[chess.square(file, rank)]

            assert actual == expected

    def test_hanging_piece_detected(self, encoder: ExtendedBoardEncoder) -> None:
    ...
    def test_mobility_planes_normalized(self, encoder: ExtendedBoardEncoder) -> None:
