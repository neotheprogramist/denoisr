from denoisr_chess.apps.train_phase1 import _evaluate_stability_guard
...
def test_stability_guard_triggers_on_loss_explosion_with_grad_spike() -> None:
    decision = _evaluate_stability_guard(
        enabled=True,
        epoch_num=12,
        best_top1=0.40,
    ...
    assert decision.trigger is True
    assert "loss exploded" in decision.reason
