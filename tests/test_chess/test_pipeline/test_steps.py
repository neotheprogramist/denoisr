...
def test_run_python_module_maps_sigint_exit_to_keyboard_interrupt() -> None:
    ...
    with patch(
        ...
        side_effect=subprocess.CalledProcessError(
            returncode=130,
            cmd=["python", "-m", "denoisr_chess.apps.train_phase1"],
        ),
    ):
        with pytest.raises(KeyboardInterrupt):
            _run_python_module("denoisr_chess.apps.train_phase1", [])
...
def test_run_python_module_forces_tqdm_off() -> None:
    ...
    with patch("subprocess.run", side_effect=_fake_run) as mock_run:
        _run_python_module("denoisr_chess.apps.train_phase1", ["--epochs", "1"])
        mock_run.assert_called_once()
...
def test_generate_data_calls_generate(tmp_path: Path) -> None:
    ...
    with patch(
        "denoisr_chess.apps.generate_data.generate_to_file", return_value=100
    ) as mock_gen:
...
def test_generate_data_fails_when_legacy_artifact_missing_metadata(tmp_path: Path) -> None:
    ...
    with patch("denoisr_chess.apps.generate_data.generate_to_file") as mock_gen:
        with pytest.raises(
            ValueError,
...
def test_train_phase1_updates_state(tmp_path: Path) -> None:
    ...
    def _fake_run(cmd: list[str], check: bool, env: dict[str, str]) -> None:
        assert check
        assert "denoisr_chess.apps.train_phase1" in cmd
...
def test_train_phase2_updates_state(tmp_path: Path) -> None:
    ...
    def _fake_run(cmd: list[str], check: bool, env: dict[str, str]) -> None:
        assert check
        assert "denoisr_chess.apps.train_phase2" in cmd
        assert "--tqdm" not in cmd
        assert env["DENOISR_TQDM"] == "0"
...
def test_train_phase3_updates_state(tmp_path: Path) -> None:
    ...
    def _fake_run(cmd: list[str], check: bool, env: dict[str, str]) -> None:
        assert check
        assert "denoisr_chess.apps.train_phase3" in cmd
        assert "--tqdm" not in cmd
        assert env["DENOISR_TQDM"] == "0"
...
def test_state_persists_full_pipeline_phases(tmp_path: Path) -> None:
    ...
    def _fake_run(cmd: list[str], check: bool, env: dict[str, str]) -> None:
        ...
        assert "--tqdm" not in cmd
        assert env["DENOISR_TQDM"] == "0"
        if "denoisr_chess.apps.train_phase1" in cmd_str:
            (output_dir / "phase1.pt").write_bytes(b"phase1")
            return
        if "denoisr_chess.apps.train_phase2" in cmd_str:
            (output_dir / "phase2.pt").write_bytes(b"phase2")
            return
        if "denoisr_chess.apps.train_phase3" in cmd_str:
            (output_dir / "phase3.pt").write_bytes(b"phase3")
            return
